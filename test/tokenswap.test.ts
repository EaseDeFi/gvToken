...
describe("TokenSwap", function () {
  ...
  describe("Initialize", function () {
    it("should initialize contract properly", async function () {
      expect(await contracts.tokenSwap.armorToken()).to.be.equal(
        contracts.armorToken.address
      );
      expect(await contracts.tokenSwap.easeToken()).to.be.equal(
  ...
  describe("swap()", function () {
    it("should allow user to swap ease tokens for armor token", async function () {
      ...
      await contracts.armorToken
        .connect(signers.user)
        .approve(contracts.tokenSwap.address, amount);
      const userEaseBalBefore = await contracts.easeToken.balanceOf(
        userAddress
      );
...
describe("EaseToken", function () {
  ...
  describe("mint()", function () {
    ...
    it("should not allow non minter to mint ease token", async function () {
      ...
      const userAddress = signers.user.address;
      await expect(
        contracts.easeToken.connect(signers.gov).mint(userAddress, amount)
